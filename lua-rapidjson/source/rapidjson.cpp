...
class Encoder
{
  ...
  private:
	...
	static bool isInteger(lua_State *L, int idx, int64_t *out)
	{
		...
		{
			if (std::numeric_limits<lua_Integer>::min() <= intpart && intpart <= std::numeric_limits<lua_Integer>::max())
			{
				*out = (int64_t)intpart;
				return true;
			}
	...
	static bool hasJsonType(lua_State *L, int idx, bool &result, const char *jsontype)
	{
		bool has = false;
		if (lua_getmetatable(L, idx))
		{
			...
			if (lua_isstring(L, -1))
			{
				...
				result = (s != NULL && strncmp(s, jsontype, strlen(jsontype)) == 0);
				has = true;
			}
			lua_pop(L, 2); // []
		}
	...
	{
		bool isarray = false;
		if (hasJsonType(L, idx, isarray, "array")) // any table with a meta field __jsontype set to 'array' are arrays
			return isarray;

		return (lua_rawlen(L, idx) > 0); // any table has length > 0 are treat as array.
	}

	static bool isObject(lua_State *L, int idx)
	{
		bool isobject = false;
		if (hasJsonType(L, idx, isobject, "object")) // any table with a meta field __jsontype set to 'object' are objects
			return isobject;

		int top = lua_gettop(L);
		lua_pushnil(L);
		while (lua_next(L, -2) != 0 && !isobject)
		{
			isobject = lua_type(L, -2) == LUA_TSTRING;
			lua_pop(L, 1);
		}
		lua_settop(L, top);

		return isobject; // any table has string type keys are treat as array.
	}
